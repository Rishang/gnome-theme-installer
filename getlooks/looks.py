...
# local
from looks_path import STATE_PATH
from utils import JsonState, JsonState, dn_n_extract, print_table, ls
from core import ProductInfo, DeskEnv, scrapGnomeLooks, message, product_state_cache
...
if os.environ.get("TEST_THEME_INSTALLER") == "true":
    ...
    state_path = f"{desk.HOME}/{STATE_PATH}"
...
def looks_rm(themes: bool = False, icons: bool = False) -> list[str]:
    ...
    state.cache = [dataclasses.asdict(product) for product in products]
    state.save()
...
def looks_log(product: ProductInfo):
    ...
    if len(cache_products) != 0:
        for i in range(len(cache_products)):
    ...
    # print(json.dumps(cache_product, indent=4))
    state.save()
...
def looks_update(theme_type: str = ""):
    ...
    for _c_product in cache_products:
        ...
        for c_fi in _c_product.files:
            ...
            for l_fi in product.files:
                if l_fi.name == c_fi.name:
                    ...
                    # check if dir exist before update
                    # for ef in c_fi.extracted_files:
                    #     if not os.path.isdir(product.install_path+ '/' +ef):
                    #         ...
        ...
        if len(state.cache) != 0:
            for i in range(len(state.cache)):
                if state.cache[i]["url"] == product.url:
                    _d = dataclasses.asdict(product)
                    state.put(_d, index=i)
