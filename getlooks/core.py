...
# local
from utils import print_table, message
from looks_path import DESK_THEME_PATH
...
class FileInfo:
    ...
    downloads: int
    changed_at: str
    extracted_files: Optional[List] = field(default_factory=list)
...
class DeskEnv:
    ...
    def check_env(self):
        ...
        if XDG_DESK:
            ...
            if "gnome" in ps:
                self.DESK_ENV = desk_env["1"]
            elif "plasma" in ps or "kde" in ps:
                self.DESK_ENV = desk_env["2"]
            elif "xfce" in ps:
                self.DESK_ENV = desk_env["3"]
        else:
...
def product_state_cache(data: list[dict]) -> list[ProductInfo]:

    products: list[ProductInfo] = []
    for p in data:
        product = ProductInfo(**p)
        product.files = [FileInfo(**f) for f in p["files"]]
