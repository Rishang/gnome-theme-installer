...
class JsonState:
    ...
    def add(self, data: dict, dublicate: bool = False):
    ...
    def put(self, data: dict, index: int) -> bool:
        # , where:dict={}
        ...
        if index != None:
            keys = self.cache[index].keys()

        if not keys == data.keys():
            print("old keys: ", keys)
            print("new keys: ", data.keys())
            raise ValueError("previos index datakeys not match")
